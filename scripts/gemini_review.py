...
리뷰가 조용히 끝나는 것보다 중요한 회귀 위험을 드러내는 것이 더 중요합니다.
확정적인 컴파일 오류가 아니어도, 변경된 라인과 diff_context에서 근거가 보이는 도메인 규칙 위반, 경계 조건 누락, 테스트 누락은 medium 이상으로 리뷰하세요.
...
- 테스트로 확인해야 하는 중요한 회귀 가능성 또는 새 도메인 분기
- src/ChaosChess.AI 동작 변경에 대응하는 xUnit 회귀 테스트 누락
- README에 명시된 ChaosChess.AI 책임 범위와 다르게 게임 상태 적용, Unity 어댑터 책임, 외부 엔진 실행을 라이브러리 내부로 가져오는 변경

리뷰하지 않을 것:
- 코드 스타일, 취향, 사소한 이름 변경
- 단순 요약, 칭찬, 일반적인 개선 제안
- diff_context나 변경 라인에서 근거를 찾을 수 없는 추측
- 같은 원인의 중복 댓글
- low 심각도의 근거가 약하거나 사소한 내용
- 테스트 누락을 지적할 때는 어떤 입력, 경계값, 도메인 상태를 검증해야 하는지 구체적으로 설명할 수 없는 경우
