...
public sealed class PeaceZoneCardTargetStrategyTests
{
    private readonly PeaceZoneCardTargetStrategy strategy = new();

    [Fact]
    public void Decide_SelectsActorEngineDestination()
    {
    ...
    [Fact]
    public void Decide_ScoresActorEngineAdjacent()
    {
        var actorPawn = Piece(PieceKind.Pawn, PieceColor.White, new Square(4, 1));
        var occupiedDestination = Piece(PieceKind.Pawn, PieceColor.Black, new Square(4, 3));
        GameState state = State(
            PieceColor.White,
            pieces: new[] { actorPawn, occupiedDestination });
        ...
        Assert.Equal(3, ComponentValue(result.SelectedCandidate.Score, "peace.actor_engine_adjacent"));
    }

    [Fact]
    public void Decide_BuffersEnemyCaptureAroundThreatenedActorPiece()
    {
        var enemyKnight = Piece(PieceKind.Knight, PieceColor.Black, new Square(6, 5), "n");
        var threatenedActorPiece = Piece(PieceKind.Pawn, PieceColor.White, new Square(4, 4));
        GameState state = State(
            PieceColor.White,
            pieces: new[] { enemyKnight, threatenedActorPiece });
        ...
        Assert.True(result.HasSelection);
        Assert.NotEqual(new Square(4, 4), result.SelectedCandidate!.Plan.Target.Squares[0]);
        Assert.Equal(6, ComponentValue(result.SelectedCandidate.Score, "peace.enemy_capture_buffer"));
    }
    ...
    public void Decide_UsesCenterControlWithoutEngineMove()
    {
        ...
        Assert.Equal(2, ComponentValue(result.SelectedCandidate.Score, "peace.center_control"));
    }
        ...
        PieceInfo[]? pieces = null)
    {
        return new GameState(
            new BoardState(
                ...
                halfmoveClock: 0,
                fullmoveNumber: 1),
            new[] { new CardInfo("peace_zone", "BoardControl", 1) },
